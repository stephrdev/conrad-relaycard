...
from .constants import (CMD_SETUP, CMD_GETPORT, CMD_SETPORT, CMD_TOGGLE,
    CMD_SETSINGLE, CMD_DELSINGLE, RESP_GETPORT, RESP_SETPORT, RESP_TOGGLE,
    RESP_SETSINGLE, RESP_DELSINGLE)
from .frame import ResponseFrame, RequestFrame
from .state import RelayState


class RelayCard(object):
    port = None
    card_count = 0
    ...
    def get_serial_port(self, port=None):
        if not hasattr(self, '_serial_port'):
            ...
            self._serial_port = serial.Serial(port or self.port,
                baudrate=19200,
    ...
    def get_ports(self, address):
        response = self.execute_retry(
            CMD_GETPORT,
            lambda r: r.command == RESP_GETPORT,
            address
        )
    ...
    def set_port(self, address, port, port_state):
        ...
        response = self.execute_retry(
            ...
            lambda r: ((port_state and r.command == RESP_SETSINGLE)
